...
describe('Comment Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should call CrimeAlert query and add missing value', () => {
      const comment: IComment = { id: 456 };
      const crime: ICrimeAlert = { id: 4964 };
      comment.crime = crime;

      const crimeAlertCollection: ICrimeAlert[] = [{ id: 51947 }];
      jest.spyOn(crimeAlertService, 'query').mockReturnValue(of(new HttpResponse({ body: crimeAlertCollection })));
      const additionalCrimeAlerts = [crime];
      const expectedCollection: ICrimeAlert[] = [...additionalCrimeAlerts, ...crimeAlertCollection];
      jest.spyOn(crimeAlertService, 'addCrimeAlertToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ comment });
      comp.ngOnInit();

      expect(crimeAlertService.query).toHaveBeenCalled();
      expect(crimeAlertService.addCrimeAlertToCollectionIfMissing).toHaveBeenCalledWith(
        crimeAlertCollection,
        ...additionalCrimeAlerts.map(expect.objectContaining)
      );
      expect(comp.crimeAlertsSharedCollection).toEqual(expectedCollection);
    });
    ...
    it('Should update editForm', () => {
      ...
      comment.user = user;
      const crime: ICrimeAlert = { id: 84499 };
      comment.crime = crime;
      ...
      expect(comp.crimeAlertsSharedCollection).toContain(crime);
  ...
  describe('Compare relationships', () => {
    ...
    describe('compareCrimeAlert', () => {
      it('Should forward to crimeAlertService', () => {
        const entity = { id: 123 };
        const entity2 = { id: 456 };
        jest.spyOn(crimeAlertService, 'compareCrimeAlert');
        comp.compareCrimeAlert(entity, entity2);
        expect(crimeAlertService.compareCrimeAlert).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
