...
type RestOf<T extends IChatMessage | NewChatMessage> = Omit<T, 'sentDate'> & {
...
export class ChatMessageService {
  ...
  protected convertDateFromClient<T extends IChatMessage | NewChatMessage | PartialUpdateChatMessage>(chatMessage: T): RestOf<T> {
    return {
      ...chatMessage,
      sentDate: chatMessage.sentDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restChatMessage: RestChatMessage): IChatMessage {
    return {
      ...restChatMessage,
      sentDate: restChatMessage.sentDate ? dayjs(restChatMessage.sentDate) : undefined,
    };
  }
