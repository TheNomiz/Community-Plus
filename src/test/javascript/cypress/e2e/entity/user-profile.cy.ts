...
describe('UserProfile e2e test', () => {
  ...
  const userProfileSample = {
    username: 'Market Chief Bedfordshire',
    name: 'payment Computer responsive',
    password: 'mesh stable turquoise',
    occupation: 'York Data',
  };
  ...
  describe('new UserProfile page', () => {
    ...
    it('should create an instance of UserProfile', () => {
      ...
      cy.get(`[data-cy="name"]`).type('grey Fish circuit').should('have.value', 'grey Fish circuit');

      cy.get(`[data-cy="password"]`).type('CottonXXXX').should('have.value', 'CottonXXXX');

      cy.get(`[data-cy="occupation"]`).type('green circuit').should('have.value', 'green circuit');
